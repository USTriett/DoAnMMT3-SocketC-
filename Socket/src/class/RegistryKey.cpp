...
{
	HKEY hKey;
	if (RegOpenKeyExW(this->hkey, subkey.c_str(), 0, KEY_READ|KEY_WOW64_64KEY, &hKey) != ERROR_SUCCESS)
	{
		return NULL;
	}
	else
...
{
	HKEY hKey;
	if (RegOpenKeyExW(this->hkey, subkey.c_str(), 0, KEY_READ|KEY_WOW64_32KEY, &hKey) != ERROR_SUCCESS)
	{
		return NULL;
	}
	else
...
{
	HKEY hKey;
	if (RegOpenKeyExW(this->hkey, subkey.c_str(), 0, KEY_READ, &hKey) != ERROR_SUCCESS)
	{
		return NULL;
	}
	else
...
RegistryKey* RegistryKey::OpenSubKey(wstring subkey, Arch a)
{
	...
	if (RegOpenKeyExW(this->hkey, subkey.c_str(), 0, KEY_READ|FLAG, &hKey) != ERROR_SUCCESS)
	{
		return NULL;
	}
	else
...
vector<wstring> RegistryKey::GetSubKeyNames()
{
	...
	while( (lRet = RegEnumKeyExW(this->hkey, dwIndex, szSubKeyName, &cbName, NULL,NULL, NULL, NULL)) != ERROR_NO_MORE_ITEMS )
	{
...
wstring RegistryKey::GetValue(wstring query)
{
	...
	DWORD dwType;
	if (RegQueryValueExW(this->hkey, query.c_str(), NULL, &dwType, (LPBYTE)&Value, &dwSize) == ERROR_SUCCESS)
    {
		if(dwType==REG_DWORD)
		{
		...
		else if(dwType==REG_EXPAND_SZ)
		{
			...
			ExpandEnvironmentStringsW(Value, Expanded, MAX_PATH);
